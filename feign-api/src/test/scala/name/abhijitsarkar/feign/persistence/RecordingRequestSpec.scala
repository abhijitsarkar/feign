...
class RecordingRequestSpec extends FlatSpec with Matchers {
  "recording request" should "copy properties from given request" in {
    ...
    recordingRequest.queryParams shouldEqual request.queryParams
    recordingRequest.headers shouldEqual request.headers
    recordingRequest.body shouldEqual request.body
  }
}
