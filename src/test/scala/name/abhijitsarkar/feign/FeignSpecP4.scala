...
class FeignSpecP4 extends FlatSpec with Matchers {
  @Autowired
  ...
  "feign" should "disable recording" in {
    ...
    response = restTemplate.exchange(s"http://localhost:$port/requests/1", GET, null, classOf[String])
